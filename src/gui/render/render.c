...
void flushLayer(guiContext *ctx, guiLayer *lay) {
  ...
  glUniform2f(glGetUniformLocation(ctx->gl.shd->program, "uScreenSize"),
      ctx->win->width,
      ctx->win->height);
  glUniform1i(glGetUniformLocation(ctx->gl.shd->program, "uTex"), 0);

  // issue draw call
  glDrawArraysInstanced(GL_TRIANGLES, 0, 6, lay->queue.last);
  GL_ERR("gui drawing")
}
...
int newGui(guiContext *ctx) {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(quadTris), quadTris, GL_STATIC_DRAW);
  GL_ERR("gui VBO data")
  ...
  glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(quad), (void *)0);
...
void scrollCallback(
    GLFWwindow *win, double x __attribute__((unused)), double y) {
  guiContext *ctx = glfwGetWindowUserPointer(win);
