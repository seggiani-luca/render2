...
int fieldGui(guiContext *ctx,
  ...
  // push panel
  quadGui(ctx, SCROLL, (float4){1 PAD, 1 PAD, WIN - 2 PAD, ROW}, BG_LIGHT);

  // push icon
  iconGui(ctx, SCROLL, (float2){2 PAD, 2 PAD + HPAD}, ico);

  // push name
  textGui(ctx, SCROLL, (float2){3 PAD + ICO_SIZ, 2 PAD + HPAD}, name);

  // push edit box
  gui(ctx,
      SCROLL,
      (float4){2 PAD, 1 PAD + HROW, WIN - 4 PAD, HROW - 1 PAD},
      val);

  // push delete button
  if (buttonGui(ctx,
          SCROLL,
          (float4){WIN - 4 PAD - ICO_SIZ,
              2 PAD,
              2 PAD + ICO_SIZ,
...
int intFieldGui(const field *f, guiContext *ctx) {
  return fieldGui(ctx, f->name, ICO_INT, intGui, &((intField *)f)->val);
}

int floatFieldGui(const field *f, guiContext *ctx) {
  return fieldGui(ctx, f->name, ICO_FLOAT, floatGui, &((floatField *)f)->val);
}
...
void addFieldGui(window *win) {
  ...
  static const fieldButton fieldButtons[] = {{"New Int", ICO_INT, intNew},
  ...
  float scrollRange = (TXT_HEIGHT + 3 PAD) * fieldButtonCount // buttons
...
renderCallback makeAddFieldCallback(entity *ent) {
  ...
  *eCtx->name = '\0';
  ...
  // return callback
  return (renderCallback){addFieldGui, eCtx, freeGui};
}
...
void entityGui(window *win) {
  ...
  // push background
  quadGui(ctx, BACKGROUND, (float4){0, 0, WIN, INSPECTOR_HEIGHT}, BG_ABS);
  ...
  // push entity label
  {
    ...
    // push icon
    iconGui(ctx, FIXED, (float2){2 PAD, 2 PAD}, ICO_ENTITY);
  ...
  // scroll field layer
  float scrollRange = (ent ? (ROW + 1 PAD) * ent->fieldCount : 0) // fields
...
renderCallback makeEntityCallback(entity *ent) {
  ...
  // return callback
  return (renderCallback){entityGui, eCtx, freeGui};
}
